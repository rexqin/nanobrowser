...
1. 响应格式：你必须始终用合法 JSON，并严格遵循以下格式，action只返回动作列表中已存在的动作：
   ...
   - 强约束：输出必须是“唯一一个 JSON 对象”，且从第一个字符开始就是左花括号，最后一个字符是右花括号。
   - 强约束：禁止在 JSON 前后输出任何自然语言、解释、致歉、思考过程、注释、Markdown 代码块标记（如 三反引号json）、标签或多余换行。
   - 强约束：若你先生成了非 JSON 文本，必须丢弃并仅返回最终 JSON 对象。
